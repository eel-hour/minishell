...
/*   Updated: 2023/07/22 01:44:05 by eel-hour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char **replacing(char **str, int trigger)
{
	int i;
	int k;
...
char *replace_pwd(char *str)
{
	...
	if (trigger)
	{
		...
		if (str[0] == '$')
			trigger_2 = 1;
		returnd = ft_split(str, '$');
		returnd = replacing(returnd, trigger_2);
...
int main()
{
	char p[100] = "echo \'$PWD\' $PWD eee$PWD$OLDPWD";
	char **s = parser(p);
	int i = 0;
	while (s[i] != 0)
		printf("%s\n",s[i++]);
